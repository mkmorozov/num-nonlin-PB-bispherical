...
# df/deta |_eta=eta1 = const / ( cosh eta - cos xi ) * sigma1 / dlen 
# (the constant charge at the first sphere surface; note the scaling due to the change of coordinates),
# df/deta |_eta=eta2 = const / ( cosh eta - cos xi ) * -sigma2 / dlen 
# (the constant charge at the second sphere surface; note the scaling due to the change of coordinates),
#
...
sigma1 = str2num( argList{3} ); # charge on the first sphere surface,
sigma2 = str2num( argList{4} ); # charge on the second sphere surface;

#for sr1 = [0.05:0.1:1] #
for sr1 = [0.02:0.02:0.28, 0.3:0.05:1.5]

bsCoef = ( sr1 * ( 2 + sr1 ) * ( 2 * r2r1 + sr1 ) * ( 2 + 2 * r2r1 + sr1 ) )^0.5 / ( 2 * ( 1 + r2r1 + sr1 ) ); 
# const from the expression for bispherical coordinates
...
# 3) the constant charge at the second sphere surface;
# note the coefficients, they are necessary to reach second-order precision
# (see the mathematica notebook attached for the derivation),
dMatEta( 1:n1:n2, 2:n1:n2 ) = zeros(n1);
dMatEta2( 1:n1:n2, 1:n1:n2 ) *= 7./4.;
dMatEta2( 1:n1:n2, 2:n1:n2 ) *= 4.;
dMatEta2( 1:n1:n2, 3:n1:n2 ) = -1./2. * speye(n1) / dEta^2;
bcs = zeros( n1 );
bcs(1,:) = bcs(1,:) - ( -sigma2 / dlen ) * bsCoef ./ ( cosh( eta2 ) - cos( xis' ) ) ...
  .* ( -3. / dEta - sinh( eta2 ) ./ ( cosh( eta2 ) - cos( xis' ) ) );
# MIND THE STRUCTURE OF LAPLACES OPERATOR IN BISPHERICAL COORDINATES!
# Also note that derivative is given, hence lower powers of dEta in the denominator.

# 4) the constant charge at the first sphere surface,
# note the coefficients, they are necessary to reach second-order precision
# (see the mathematica notebook attached for the derivation),
dMatEta( n1:n1:n2, n1-1:n1:n2 ) = zeros(n1);
dMatEta2( n1:n1:n2, n1:n1:n2 ) *= 7./4.;
dMatEta2( n1:n1:n2, n1-1:n1:n2 ) *= 4.;
dMatEta2( n1:n1:n2, n1-2:n1:n2 ) = -1./2. * speye(n1) / dEta^2;
bcs(n1,:) = bcs(n1,:) - ( sigma1 / dlen ) * bsCoef ./ ( cosh( eta1 ) - cos( xis' ) ) ...
  .* ( 3. / dEta - sinh( eta1 ) ./ ( cosh( eta1 ) - cos( xis' ) ) );
# MIND THE STRUCTURE OF LAPLACES OPERATOR IN BISPHERICAL COORDINATES!
# Also note that derivative is given, hence lower powers of dEta in the denominator.
